...
mkdir -p afternoon_Ypestis
cd afternoon_Ypestis
...
fastq-dump -X 200000 SRR18369392 #now we download the first 200,000 spots (all of it would take too long for the purposes of training!)
...
wget https://raw.githubusercontent.com/Imogen-D/Bases_De_Bash_Workshop/main/Ypestis_align.sh
ls #check your file is there
less Ypestis_align.sh  #whats in the file? press 'q' to quit
chmod +x Ypestis_align.sh #make the script executable
...
sbatch --cpus-per-task=1 --mem=4G  -J align.$output_name -o align.${output_name}.log Ypestis_align.sh SRR18369392.fastq GCF_015336265.1_ASM1533626v1_genomic.fna.gz  $output_name #run the script, giving it the genome and the fastq file
...
#first check the log file (always important to make sure everything ran correctly) 
less align.${output_name}.log #press q to quit

#and then look at the table of coverage
less ${output_name}_aligned_sorted_coveragetables.txt #press q to quit

#the SNP calling is a big file, how big?
wc -l ${output_name}_aligned_sorted_snps.vcf

#lets scroll through it, press 'q' to quit
less ${output_name}_aligned_sorted_snps.vcf
