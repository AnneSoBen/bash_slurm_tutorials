...
samtools view -S -b results/${name}_aligned.sam > results/${name}_aligned.bam

#we have to sort the alignment
echo sorting the alignment
samtools sort -o results/${name}_aligned_sorted.bam results/${name}_aligned.bam 

#we can look at a histogram of coverage
echo making the coverage table
samtools coverage -m -o results/${name}_aligned_sorted_coveragetables.txt results/${name}_aligned_sorted.bam
...
echo snp calling
bcftools mpileup -Ou -f $genome.bgz results/${name}_aligned_sorted.bam | bcftools call -mv -Ov -o results/${name}_aligned_sorted_snps.vcf
